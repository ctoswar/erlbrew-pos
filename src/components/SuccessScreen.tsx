  ...
  return (
    <div className="flex-1 flex items-center justify-center bg-erl-base relative overflow-hidden">
      ...
      <div className="animate-success-pop text-center px-6 relative max-w-md mx-auto">
        {/* Success Mark */}
        <div className="relative mx-auto mb-8 w-20 h-20 sm:w-24 sm:h-24 md:w-28 md:h-28">
          {/* Outer glow ring */}
          <div className="absolute -inset-3 sm:-inset-4 rounded-full bg-erl-success/5 animate-pulse-slow" style={{ boxShadow: '0 0 60px rgba(122,191,122,0.1)' }} />
          {/* Success circle */}
          <div className="w-full h-full rounded-full bg-gradient-to-br from-erl-success/15 to-erl-success/[0.02] border-2 border-erl-success/25 flex items-center justify-center shadow-[0_0_60px_rgba(122,191,122,0.08)]">
        ...
        <div className="font-display text-2xl sm:text-3xl text-erl-success mb-3 tracking-wide font-bold">Order Placed!</div>

        {/* Order ID */}
        <div className="animate-scale-in card-glass inline-block py-3 px-5 sm:py-4 sm:px-7 rounded-2xl mb-3">
          <div className="text-[10px] text-erl-text-muted tracking-[3px] uppercase mb-1.5 font-bold">Order ID</div>
          <div className="text-lg sm:text-xl font-bold text-erl-accent tracking-[0.2em] font-mono">
            {order.id}
          </div>
        </div>
        ...
        <div className="flex flex-col sm:flex-row gap-3 justify-center mt-8">
          <button className="btn btn-accent text-[10px] py-3 px-5 min-h-[44px] w-full sm:w-auto shadow-lg hover:shadow-xl transition-all rounded-xl tracking-[0.1em]" onClick={() => setShowPreview(true)}>
            🖨 Print Receipt
          </button>
          <button className="btn btn-outline text-[10px] py-3 px-5 min-h-[44px] w-full sm:w-auto rounded-xl tracking-[0.1em]" onClick={handlePrintAndDone}>
            Print & Done
          </button>
          {onRepeat && (
            <button className="btn btn-outline text-[10px] py-3 px-5 min-h-[44px] w-full sm:w-auto rounded-xl tracking-[0.1em]" onClick={onRepeat}>
              🔁 Repeat Order
            </button>
          )}
          <button className="btn btn-outline text-[10px] py-3 px-5 min-h-[44px] w-full sm:w-auto rounded-xl tracking-[0.1em]" onClick={onDone}>
            New Order
          </button>
        </div>
      </div>
