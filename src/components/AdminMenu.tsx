            ...
            <div style={{
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                ...
                <FormField label="Emoji">
                  <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
                    {EMOJIS.map((e) => (
                      <button key={e} onClick={() => setField("emoji", e)}
                        style={{
              ...
              <div style={{ display: "flex", gap: 8, marginTop: 18 }}>
                <button onClick={closeForm}
                  style={{ flex: 1, padding: "11px 0", borderRadius: 9, border: "1px solid var(--border-default)", background: "transparent", color: "var(--text-secondary)", fontSize: 10, fontWeight: 700, letterSpacing: 1, cursor: "pointer", textTransform: "uppercase" as const }}>
                  Cancel
                </button>
                <button onClick={handleSave} disabled={saving}
                  style={{ flex: 1, padding: "11px 0", borderRadius: 9, border: "none", background: "var(--gold)", color: "var(--bg-sidebar)", fontSize: 10, fontWeight: 700, letterSpacing: 1, cursor: saving ? "not-allowed" : "pointer", textTransform: "uppercase" as const, opacity: saving ? 0.6 : 1 }}>
...
const AdminItemCard: React.FC<AdminItemCardProps> = ({ item, onEdit, onDelete, deleteConfirm, onConfirmDelete, onCancelDelete }) => (
  <div style={{
...
const FormField: React.FC<{ label: string; hint?: string; children: React.ReactNode }> = ({ label, hint, children }) => (
  <div>
    <div style={{ fontSize: 9, color: "var(--gold-muted)", letterSpacing: 1.5, marginBottom: 5, textTransform: "uppercase" as const, fontWeight: 700 }}>{label}{hint && <span style={{ fontWeight: 400, color: "var(--text-faint)", textTransform: "none" as const, marginLeft: 4 }}>{hint}</span>}</div>
    {children}
  </div>
);